...
function updateNavbarState() {
  ...
  const ctaMobile = document.getElementById('nav-cta-mobile');
  ...
  if (ctaMobile) {
    if (user) {
      ctaMobile.innerHTML = `
        <span style="font-size:0.9rem; font-weight:600; text-align:center; display:block; margin-bottom: 8px;">Hai, ${user.fullName.split(' ')[0]}</span>
        <a href="profile.html" class="btn-ghost w-full text-center" style="margin-bottom: 8px; display:block;">Profil</a>
        <a href="app.html" class="btn-primary w-full text-center" style="display:block;">Lanjut Asesmen</a>
      `;
    } else {
      ctaMobile.innerHTML = `
        <a href="auth.html" class="btn-ghost w-full text-center" style="margin-bottom: 8px; display:block;">Login</a>
        <a href="auth.html" class="btn-primary w-full text-center" style="display:block;">Mulai Asesmen</a>
      `;
    }
  }
}
